...
' Main function to create the OnEdge Business Goals and Objectives Diagram
Sub CreateBusinessGoalsDiagram()
    ...
    Session.Output "VBScript CREATE ONEDGE BUSINESS GOALS AND OBJECTIVES DIAGRAM"
    Session.Output "======================================="
    ...
    ' Check for existing Business Goals and Objectives diagram
    Dim goalsDiagram As EA.Diagram
    Set goalsDiagram = Nothing
    For i = 0 To phaseAPackage.Diagrams.Count - 1
        Dim diag As EA.Diagram
        Set diag = phaseAPackage.Diagrams.GetAt(i)
        If diag.Name = "OnEdge Business Goals and Objectives" Then
            Set goalsDiagram = diag
            Exit For
        End If
    Next

    ' Create Business Goals and Objectives diagram if it doesn't exist
    If goalsDiagram Is Nothing Then
        Set goalsDiagram = phaseAPackage.Diagrams.AddNew("OnEdge Business Goals and Objectives", "Class")
        If Not goalsDiagram Is Nothing Then
            goalsDiagram.Notes = "Business Goals and Objectives Diagram for OnEdge AI Intelligence Service"
            goalsDiagram.Update
            Session.Output "Created diagram: OnEdge Business Goals and Objectives"
        Else
            Session.Output "Error: Failed to create OnEdge Business Goals and Objectives diagram."
            hasError = True
            Exit Sub
        End If
    Else
        Session.Output "Diagram already exists: OnEdge Business Goals and Objectives"
    End If

    ' Define elements (goals, components, industries) with names, stereotypes, colors, and positions
    Dim elements(18)
    Dim stereotypes(18)
    Dim elementTypes(18)
    Dim elementObjects(18)
    Dim elementColors(18)
    Dim elementSizes(18)
    Dim elementCount
    elementCount = 0

    ' Business Goals (Green/Blue, larger size)
    ...
    elements(1) = "Low Latency": stereotypes(1) = "Goal": elementTypes(1) = "Class": elementColors(1) = RGB(0, 128, 0): elementSizes(1) = "l=300;r=500;t=50;b=0" ' Green
    elementCount = elementCount + 1
    elements(2) = "Affordability": stereotypes(2) = "Goal": elementTypes(2) = "Class": elementColors(2) = RGB(0, 102, 204): elementSizes(2) = "l=550;r= kand750;t=50;b=0" ' Blue
    elementCount = elementCount + 1
    elements(3) = "Data Security": stereotypes(3) = "Goal": elementTypes(3) = "Class": elementColors(3) = RGB(0, 102, 204): elementSizes(3) = "l=800;r=1000;t=50;b=0" ' Blue
    elementCount = elementCount + 1
    elements(4) = "Scalability": stereotypes(4) = "Goal": elementTypes(4) = "Class": elementColors(4) = RGB(0, 128, 0): elementSizes(4) = "l=50;r=250;t=150;b=100" ' Green
    elementCount = elementCount + 1
    elements(5) = "Sustainability": stereotypes(5) = "Goal": elementTypes(5) = "Class": elementColors(5) = RGB(0, 128, 0): elementSizes(5) = "l=300;r=500;t=150;b=100" ' Green
    elementCount = elementCount + 1
    elements(6) = "Customization": stereotypes(6) = "Goal": elementTypes(6) = "Class": elementColors(6) = RGB(0, 102, 204): elementSizes(6) = "l=550;r=750;t=150;b=100" ' Blue
    elementCount = elementCount + 1

    ' Enabling Components (Purple)
    ...
    Set diagObjects = goalsDiagram.DiagramObjects
    For i = 0 To elementCount - 1
    ...
    ' Create associations based on Business Goals and Objectives Diagram
    Dim connectors As EA.Collection
    Set connectors = phaseAPackage.Connectors
    Dim diagLinks As EA.Collection
    Set diagLinks = goalsDiagram.DiagramLinks
    ...
    ' Goals to Industries (Dashed lines for indirect impact)
    Dim goals, industries
    For goals = 0 To 6 ' All goals
        For industries = 12 To 16 ' All industries
            Set conn = connectors.AddNew("", "Association")
            conn.SupplierID = elementObjects(goals).ElementID
            ...
            link.Update
            Session.Output "Created association: " & elements(goals) & " -> " & elements(industries) & " (Supports, Dashed)"
        Next
    Next
    ...
    goalsDiagram.Update
    Repository.ReloadDiagram goalsDiagram.DiagramID
    Session.Output "OnEdge Business Goals and Objectives created successfully under MODEL > Architecture Development Method > Phase A"
    Session.Output "Done!"
End Sub
